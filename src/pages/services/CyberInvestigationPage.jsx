...
import {
  ShieldCheck, Scale, Landmark, ArrowRight, CheckCircle2, Quote, Building2, Banknote, FileWarning, ShieldAlert, Bug, Radar as RadarIcon, Network, FileClock, ChevronDown, Siren,
} from "lucide-react";
...
/* ----------------------------------------------------------------------
   FORFRA SOLUTIONS — CYBER INVESTIGATION PAGE
   ...
   Each service step is a toggle key — tap to expand/collapse the detail.
------------------------------------------------------------------------- */
...
export default function CyberInvestigationPage() {
  ...
  const ci = t.cyberInvestigation;
  const SERVICES = SERVICES_META.map((m, i) => ({ ...m, ...ci.services[i] }));
  const CLIENT_TYPES = CLIENT_TYPES_META.map((m, i) => ({ ...m, label: ci.clientTypes[i] }));
  const WHY_POINTS = ci.whyPoints;

  const [navSolid, setNavSolid] = useState(false);
  const [scrollPct, setScrollPct] = useState(0);
  const [openStep, setOpenStep] = useState(null); // toggle key — which step is expanded
  const SignatureIcon = ShieldAlert;
  ...
  return (
    <div className="fa-root">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .fa-step-body { background:#fff; border:1px solid #e7e2d4; border-radius:12px; padding:22px 26px; flex:1;
          box-shadow: 0 2px 10px rgba(14,36,82,0.04); transition: box-shadow .3s ease, transform .3s ease; cursor:pointer; }
        ...
        .fa-step-head { display:flex; align-items:flex-start; justify-content:space-between; gap:14px; }
        ...
        /* ---------- STEP TOGGLE KEY ---------- */
        .fa-step-toggle {
          background:none; border:none; padding:4px; margin:2px 0 0; cursor:pointer;
          color: var(--gold); flex-shrink:0; display:flex; align-items:center; justify-content:center;
          border-radius:50%; transition: background .25s ease, transform .3s ease;
        }
        .fa-step-toggle:hover { background: rgba(216,169,59,0.12); }
        .fa-step-toggle.open { transform: rotate(180deg); }
        .fa-step-detail {
          max-height:0; opacity:0; overflow:hidden;
          transition: max-height .45s ease, opacity .4s ease, margin-top .45s ease;
        }
        .fa-step-detail.open { max-height:220px; opacity:1; margin-top:14px; }
        .fa-step-detail-inner {
          padding-top:14px; border-top:1px dashed #e7e2d4;
          color:#5b6074; font-size:0.92rem; line-height:1.7;
        }
        ...
        .fa-radar-ring { position:absolute; border-radius:50%; border:1px solid rgba(216,169,59,0.35); }
        ...
        .fa-gavel-icon { animation: fa-signal-pulse 3.4s ease-in-out infinite; transform-origin: 50% 50%; }
        @keyframes fa-signal-pulse {
          0%, 70%, 100% { transform: scale(1); opacity: 0.9; }
          80% { transform: scale(1.18); opacity: 1; }
          88% { transform: scale(0.94); opacity: 0.8; }
        }
        .fa-gavel-ring {
        ...
        @media (prefers-reduced-motion: reduce) {
          ...
          .fa-step-detail { transition: none; }
        }
      `}</style>
      ...
      <nav className={`fa-nav ${navSolid ? "solid" : ""}`}>
        ...
        <Link to="/" className="fa-nav-cta">{ci.navCta}</Link>
      </nav>
      ...
      <header className="fa-hero">
        ...
        <div className="fa-eyebrow">
          <SignatureIcon size={14} className="fa-spin-slow" /> {ci.heroEyebrow}
        </div>
        <h1>
          {ci.heroTitle1}<br />
          {ci.heroTitle2}<em>{ci.heroTitle3}</em>{ci.heroTitle4}
        </h1>
        <p className="lede">
          {ci.heroLede}
        </p>
        <div className="fa-hero-actions">
          <a href="mailto:hello@forfrasolutions.com" className="fa-btn-primary">
            {ci.heroBtnPrimary} <ArrowRight size={17} />
          </a>
        </div>
        <div className="fa-hero-stats">
          <StatCounter value={6} suffix="" label={ci.stats[0].label} />
          <StatCounter value={6} suffix="" label={ci.stats[1].label} />
          <StatCounter value={100} suffix="%" label={ci.stats[2].label} />
        </div>
      </header>
      ...
      <section className="fa-section" id="about">
        <Reveal>
          <div className="fa-kicker">{ci.introKicker}</div>
          <h2 className="fa-h2">{ci.introHeading}</h2>
        </Reveal>
        <div className="fa-intro-wrap" style={{ marginTop: 40 }}>
          ...
          <Reveal delay={160}>
            <div className="fa-intro-card fa-glow-card">
              <h3>{ci.introCardTitle}</h3>
              <p>
                {ci.introCardBody}
              </p>
              <div className="fa-intro-quote">
                <Quote size={20} />
                <span>{ci.introQuote}</span>
              </div>
            </div>
          </Reveal>
        </div>
      ...
      {/* SERVICES — each step is a toggle key: tap to expand/collapse the detail */}
      <section className="fa-section tight" id="services" style={{ background: "#fff" }}>
        <Reveal>
          <div className="fa-kicker">{ci.servicesKicker}</div>
          <h2 className="fa-h2">{ci.servicesHeading}</h2>
          <p className="fa-sub">
            {ci.servicesSub}
          </p>
        </Reveal>
        <div className="fa-process">
          ...
          {SERVICES.map((s, i) => {
            ...
            const toggle = () => setOpenStep(isOpen ? null : i);
            return (
              <Reveal key={s.n} delay={i * 90}>
                <div className="fa-step">
                  ...
                  <div
                    className="fa-step-body fa-glow-card"
                    onClick={toggle}
                    role="button"
                    tabIndex={0}
                  ...
                  >
                    <div className="fa-step-head">
                      <div>
                        <span className="fa-step-num">{ci.protocolLabel} {s.n}</span>
                        <h4>{s.title}</h4>
                        <p>{s.text}</p>
                      </div>
                      <button
                        type="button"
                        className={`fa-step-toggle ${isOpen ? "open" : ""}`}
                        aria-label={isOpen ? "Hide details" : "View key details"}
                        onClick={(e) => { e.stopPropagation(); toggle(); }}
                      >
                        <ChevronDown size={18} />
                      </button>
                    </div>
                    <div className={`fa-step-detail ${isOpen ? "open" : ""}`}>
                      <div className="fa-step-detail-inner">{s.detail}</div>
                    </div>
                  </div>
                </div>
              </Reveal>
      ...
      <section className="fa-section fa-why" id="why">
        <Reveal>
          <div className="fa-kicker">{ci.whyKicker}</div>
          <h2 className="fa-h2">{ci.whyHeading}</h2>
          <p className="fa-sub">
            {ci.whySub}
          </p>
        </Reveal>
      ...
      <section className="fa-section fa-industries-band" id="clients">
        <div className="fa-ind-head">
          <Reveal>
            <div className="fa-kicker">{ci.clientsKicker}</div>
            <h2 className="fa-h2">{ci.clientsHeading}</h2>
            <p className="fa-sub">
              {ci.clientsSub}
            </p>
          </Reveal>
